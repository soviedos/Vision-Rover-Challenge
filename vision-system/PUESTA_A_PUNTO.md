...
## 9. Verificar que el árbitro se niega, y que cierra bien

Estas dos comprobaciones necesitan la cancha montada y no se pueden hacer con
imágenes generadas. Están escritas con la mecánica ya resuelta para que el día
que se corran no haya que averiguar nada sobre la marcha.

**Por qué no alcanza con apretar `r`.** El hilo de teclado del sistema solo
arranca en una terminal interactiva, así que un comando mandado por la entrada
estándar se ignora. Para estas pruebas se usa **`--fase READY`**, que no es un
atajo: es una **intención** que pasa por la misma guarda que la tecla, se
reintenta en cada cuadro y, cuando no puede, imprime el motivo cada cinco
segundos. Para verificar una guarda es mejor que una tecla, porque la guarda
habla. La ventana igual acepta `r`, `f`, `a` y `q`.

### 9.1 — La guarda de geometría

Verifica que el sistema **se niega a preparar una ronda si no ve la cancha**.

**Arrancá con la cámara USB DESCONECTADA.** El orden importa y es al revés de lo
que parece: desconectarla *en medio* de una corrida no reproduce el caso, porque
la fuente simplemente deja de entregar cuadros. Lo que reproduce el caso real es
arrancar sin ella, para que el sistema abra **la cámara integrada de la máquina**
y se quede mirando una habitación.

```bash
.venv/bin/python -m vision.sistema --fase READY --ventana
```

Lo que tiene que pasar:

- la fase se queda en **`IDLE`**;
- cada cinco segundos aparece `[fase] esperando para preparar la ronda: no hay
  coordenadas. Hacen falta los cuatro marcadores de esquina...`;
- el panel muestra **`SIN COORDENADAS`** en rojo.

Después **conectá la USB y relanzá**. Sin tocar ninguna tecla, el sistema tiene
que pasar a `READY` solo y arrancar la cuenta de preparación.

> Esta prueba existe por una ronda real: el 11-sep-2026 el sistema corrió doce
> segundos con la cámara integrada mirando una habitación, pasó de READY a
> RUNNING a FINISHED solo y escribió un acta de 0:08. Todo el mecanismo
> funcionó; lo que faltaba era que se negara.

### 9.2 — El cierre por reto cumplido, en cancha

Verifica la cadena completa con cubos de verdad, hasta el acta.

**Los tres cubos tienen que empezar FUERA de sus zonas.** Si ya están adentro
cuando arranca `RUNNING`, la ronda **no** se cierra por reto cumplido: el cierre
exige pasar de incompleto a completo *durante* la ronda, justamente para que una
cancha que quedó armada de la vuelta anterior no dé un tiempo de un segundo. El
acta lo registraría como arranque irregular y la corrida se perdería.

```bash
.venv/bin/python -m vision.sistema --ventana
```

Con los tiempos reales: 60 s de preparación y 600 s de ronda. No lleva diez
minutos: la ronda se cierra **apenas** los tres cubos estén en posición y se
sostengan un segundo.

Lo que tiene que pasar:

- `IDLE → READY` al apretar `r`, con la cuenta regresiva en pantalla;
- `READY → RUNNING` **sola**, al llegar a cero;
- el contador de acopio subiendo a medida que entran los cubos;
- `RUNNING → FINISHED` **sola**, con motivo `reto_cumplido`, y el cronómetro
  detenido en el instante en que **entró** el último cubo —no un segundo después,
  cuando se cumple la permanencia—;
- un acta en `actas/`.

**Esa acta se guarda versionada como `vision/record/acta_ejemplo.json`.** Es la
primera de una ronda real y queda como patrón para revisar las que vengan. Es la
única excepción a que las actas no se versionen: una se conserva documentada, no
se acumulan cientos.
